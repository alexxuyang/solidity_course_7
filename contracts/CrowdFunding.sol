...
contract CrowdFunding is CrowdFundingStorage {
    ...
    event CampaignCreated(address indexed _from, uint256 _id);
    ...
    function newCampaign(address payable receiver, uint256 goal) external isOwner() returns(uint256 id) {
        id = numCampaigns++;
        Campaign storage c = campaigns[id];
        c.receiver = receiver;
        c.fundingGoal = goal;

        emit CampaignCreated(msg.sender, id);
    }
